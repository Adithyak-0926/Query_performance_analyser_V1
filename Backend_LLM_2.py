...
class DataProcessor:
    def __init__(self,queryID, mode, query, query_aliases = None):
    ...
    def traverse_json(self,json_data_dict, parent_operator_id=None, markdown_rows=None):
        if markdown_rows is None:
            markdown_rows = []
        # if not isinstance(json_data_dict,dict):
        # json_data_dict = json.loads(json_data_dict)

        operator_id = json_data_dict["operator_id"]
        operator_name = json_data_dict["operator"]
        cost_percent = json_data_dict.get("cost_percent")  # Use .get() to handle missing keys gracefully
        excluded_keys = ["operator_id", "operator", "childOperators", "cost_percent"]
        operator_stats = {key: json_data_dict[key] for key in json_data_dict if key not in excluded_keys}
    
        # Construct the Markdown row for the current operator
        markdown_row = {
            "operator_id": operator_id,
            "operator": operator_name,
            "parent_operator_id": parent_operator_id,
            "cost_percent": cost_percent,
            "operator_stats": json.dumps(operator_stats, indent=2)
        }
    
        # Append the Markdown row to the list
        markdown_rows.append(markdown_row)
        # json_data_dict=json.dumps(child)
    
        # Recursively traverse child operators if they exist
        child_operators = json_data_dict.get("childOperators", [])
        for child in child_operators:
            self.traverse_json(child, parent_operator_id=operator_id, markdown_rows=markdown_rows)

        return markdown_rows

    def write_markdown_table_to_file(self,markdown_rows, file_path):
     with open(file_path, "w") as file:
        # Write Markdown table header
        file.write("| parent_operator_id | operator_id | operator | cost_percent | operator_stats |\n")
        file.write("|--------------------|-------------|----------|--------------|----------------|\n")

        # Write each Markdown row as a table row
        for row in markdown_rows:
            parent_operator_id = row["parent_operator_id"]
            operator_id = row["operator_id"]
            operator_name = row["operator"]
            cost_percent = row["cost_percent"]
            operator_stats = row["operator_stats"]

            # Format and write the row
            file.write(f"| {parent_operator_id if parent_operator_id is not None else 'NULL'} | {operator_id} | {operator_name} | {cost_percent} | {operator_stats} |\n")
    ...
    def get_response(self, question):
     ...
     output_path = os.path.join(self.output_folder, f"{self.queryID}_{question.replace(' ', '_')}.json")
     with open(output_path,"r") as j:     
        modified_data_1 = json.load(j)
     md_rows = self.traverse_json(modified_data_1)
     self.write_markdown_table_to_file(md_rows,"json_md_test.md")
     with open("json_md_test.md","r") as k:
          modified_data = k.read()
     with open("extra_processed_metadata.json","r") as f :
         schema_n_metadata = json.load(f)
     with get_openai_callback() as cb:
        response = self.ultimate_response(question,modified_data_1, modified_data, schema_n_metadata)
    ...
    def ultimate_response(self,question,JSON_data,md_Data,schema_n_metadata):
     ...
     if (question == "Can you give me pruning analysis."):
         ...
         response = QnA_teller({'question': question, 'document': self.document, 'data': md_Data, 'query_text' : self.query, 'pruningAndPartitioningReport' : pruningAndPartitioningReport})
     else: 
         ...
         response = QnA_teller({'question': question, 'document': self.document, 'data': md_Data, 'query_text' : self.query, 'pruningAndPartitioningReport' : pruningAndPartitioningReport, 'BadDistributionReport' : distributionReport, 'badJoinReport' : badJoinReport})
     return response
...
class Pruning_Partitioning_distribution_badJoin_Analyzer:
    ...
    def generate_partitioning_report(self):
        ...
        for table_metadata in tables:
            actual_table_name = table_metadata['tableName']
            if actual_table_name in self.involved_tables:
                is_partitioned = table_metadata.get("isPartitioned", False)
                if is_partitioned:
                    partitioned_columns = []
                    for field in table_metadata.get("fields", []):
                        if field.get("isPartitionField", False):
                            partitioned_columns.append(field['name'])
